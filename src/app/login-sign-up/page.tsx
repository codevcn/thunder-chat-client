...
type TCheckUserFormProps = {
   onSetCheckUserStatus: (status: ECheckUserStatus) => void
   onSetTypedEmail: (email: string) => void
}

const CheckUserForm = ({ onSetCheckUserStatus, onSetTypedEmail }: TCheckUserFormProps) => {
   ...
   return (
      ...
      >
         <div className="Intro flex flex-col gap-y-1 items-center text-base font-roboto">
            ...
            <span className="text-center">Welcome to Thunder Chat. Enjoy!</span>
         ...
         <div className="mt-16">
            <label htmlFor="CheckUserForm-email" className="block pl-5 mb-1 font-bold">
               <span>Email</span> <span className="text-red-500">*</span>
            </label>

            <Form.Item<TCheckUserForm> name="email" rules={[{ validator: emailValidator }]}>
               <Input
                  id=""
                  type="text"
         ...
         <button className="flex font-roboto text-base gap-x-3 justify-center items-center rounded-full w-fit px-11 py-2 mt-6 bg-black text-white box-border">
            {loading ? (
...
const Background = () => {
   return (
      <div
         style={{ backgroundImage: `url(${AppLogoPng.src})`, height: "100%", width: "150px" }}
      ></div>
   )
}
         ...
         <div className="flex flex-row items-center justify-evenly text-black overflow-hidden m-auto bg-white h-5/6 pr-10 rounded max-w-2xl">
            ...
            <div className="m-auto w-[440px] pt-7 pb-7">
               {checkUserStatus === ECheckUserStatus.UNKOWN ? (
                  ...
                  <LoginForm typedEmail={typedEmail} />
               ) : (
                  checkUserStatus === ECheckUserStatus.NOT_EXIST && (
                     <RegisterForm typedEmail={typedEmail} />
                  )
               )}
            </div>
         </div>
