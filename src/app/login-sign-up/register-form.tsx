...
const Warning = ({ text }: { text: string }) => {
   return <div className="text-red-600 mt-2 w-full text-center text-xs font-bold">{text}</div>
}

const DateOfBirth = ({ onSetData }: { onSetData: TSetDataHandler }) => {
   ...
   return (
      <Form onFinish={submit}>
         <div className="mt-5 w-[440px] box-border">
            ...
            >
               <span>Date of birth</span> <span className="font-bold text-red-700">*</span>
            </label>
            ...
            {message && <Warning text="Don't empty any fields" />}

            <Form.Item className="flex justify-center">
               <button className="flex mt-5 gap-x-3 text-base justify-center items-center rounded-full w-fit px-9 py-2 bg-black text-white">
                  <span>Next</span>
      ...
      <Form onFinish={onFinish}>
         <div className="mt-5 w-[440px] p-5 pb-0">
            ...
            <div className="flex gap-x-5">
               <div>
                  <label htmlFor="RegisterForm-firstName" className="text-xs pl-2">
                     First Name
                  </label>
               ...
               <div>
                  <label htmlFor="RegisterForm-lastName" className="text-xs pl-2">
                     Last Name
                  </label>
                  <div className="relative mt-1">
                     <Form.Item<TRegisterUserForm>
                     ...
                     rules={[
                        {
                           ...
                           required: true,
                           message:
                              "Password must be from 6 characters and must contain one upper case and one number",
                        },
                     ]}
                  >
                     <Input.Password
...
const RegisterForm = ({ typedEmail }: { typedEmail: string }) => {
   ...
   const slide = () => {
      if (step === ERegisterSteps.DATE_OF_BIRTH) return "-translate-x-[0%]"
      else if (step === ERegisterSteps.NAME_PASSWORD) return "-translate-x-[50%]"
   }
      ...
      <div className="flex flex-col items-center box-border">
         ...
         <div className="w-[440px]">
            <div className={`w-min flex ${slide()} transition duration-300`}>
               <DateOfBirth onSetData={handleSetData} />

               <NameAndPassword onSwitchStep={switchStep} preData={data} />
            </div>
         </div>
      </div>
   )
